...
const wxconfig = config_1.config.weapp_auth;
async function getUserInfo(req, res, next) {
    let user_id = req.jwtAccessToken ? req.jwtAccessToken.sub : null;
    if (!user_id)
        return res.sendErr('用户过期');
    let { userInfoKey, userInfoExpire, classTotalKey, classTotalExpire, catalogLockedKey, catalogLockedExpire, classLockedKey, classLockedExpire } = config_1.config.redisCache;
    ...
    if (!(class_total || class_total == 0)) {
        class_total = await dao_1.ClassesDao.getInstance().findClassTotal();
    ...
    if (!(class_locked || class_locked == 0)) {
        class_locked = await await dao_1.RecordsDao.getInstance().findAcountClassByUserId(user_id);
    ...
    if (!(catalog_locked || catalog_locked == 0)) {
        catalog_locked = await dao_1.RecordsDao.getInstance().findAcountCatalogByUserId(user_id);
        rediscache.setRedisCache(user_id, catalog_locked, catalogLockedExpire, catalogLockedKey);
    }
    let { id, username, nickname, avatarurl, gender, country, province, city, abstract, desc, readtime, modified, created } = user;
    let data = {
...
async function setUserInfo(req, res, next) {
    ...
    if (!results)
        return res.sendErr('更新失败');
    return res.sendOk();
}
exports.setUserInfo = setUserInfo;
async function weappLogin(req, res, next) {
    let code = req.query['code'];
    if (!code)
        return res.sendErr('验证错误，缺失的code');
    let authRequest = new http_1.myHttpRequest(wxconfig.host, wxconfig.path, 'https');
    ...
    if (results.errcode)
        res.property('errcode', results.errcode);
    if (results.errmsg)
        res.property('errmsg', results.errmsg);
    return res.sendErr('请求失败');
}
exports.weappLogin = weappLogin;
async function weappSign(req, res, next) {
    let { rds_session_key, encryptedData, iv, rawData, signature } = req.body;
    if (!rds_session_key)
        return res.sendErr('请求失败，缺少参数');
    let session_key = await redis.get(`${wxconfig.session_key_pre}:${rds_session_key}`);
    if (!session_key)
        return res.sendErr('session_key异常');
...
async function findUserClassList(req, res, next) {
    ...
    let classList = await dao_1.RecordsDao.getInstance().findByUserId(options);
    if (!classList)
        return res.sendErr('获取个人信息失败');
    let data = [];
    classList.rows.forEach(el => {
        ...
        if (objs.cover_pic)
            objs.cover_pic = utils.picReplaceUrl(objs.cover_pic, config_1.config.imageHost);
        data.push(objs);
    });
    res.property('total', classList.count);
    return res.sendOk(data);
}
exports.findUserClassList = findUserClassList;
async function trySignUp(options) {
    let user = await dao_1.UsersDao.getInstance().findByUnionId(options.unionid);
    if (user)
        return user;
    let create = await dao_1.UsersDao.getInstance().createUser(options);
    if (!create)
        return null;
    return create;
}
